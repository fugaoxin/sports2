...
//        let rect = self.bounds
    ...
    lazy var pageControl: UIPageControl = {
        ...
        pageControl.pageIndicatorTintColor = UIColor.hexColor(hex: "cfcfcf")
        pageControl.isUserInteractionEnabled = false;
        pageControl.currentPageIndicatorTintColor = UIColor.hexColor(hex: "ff5c38")
    ...
    fileprivate func originIndexPath() -> IndexPath {
        if endless
        {
            ...
            if isTopHiddden == 0 {
                ...
                centerIndex = self.factNumbers / self.numbers
...
extension CWBanner {
    fileprivate func configureBanner() {
        if self.customPageControl == nil {
            ...
            self.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "V:[control(30)]-(-25)-|",
                                                               options: [],
                                                               metrics: nil,
                                                               views: ["control" : self.pageControl]))
        }else {
        ...
        NotificationCenter.default.addObserver(self,
                                               selector: #selector(appActive(_:)), name: NSNotification.Name.NSExtensionHostDidBecomeActive ,
                                               object:nil)
